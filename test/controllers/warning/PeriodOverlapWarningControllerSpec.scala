...
import views.html.warning.PeriodOverlapWarningView
  ...
  "PeriodOverlapWarning Controller" - {
    ...
    "must return OK and the correct view for a GET in NormalMode" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(controllers.claim.routes.RefundPeriodController.onPageLoad(NormalMode), NormalMode)(
          request,
    ...
    "must redirect to ContactDetailsController on submit in NormalMode" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual controllers.claim.routes.ContactDetailsController.onPageLoad(NormalMode).url
      }
    }
    ...
    "must redirect to CheckYourClaimDetailsController on submit in CheckMode" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual controllers.claim.routes.CheckYourClaimDetailsController.onPageLoad().url
      }
    }
  }
}
