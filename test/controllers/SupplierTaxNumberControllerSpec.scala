...
class SupplierTaxNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "SupplierTaxNumber Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(SupplierTaxNumberPage, SupplierTaxNumber.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(SupplierTaxNumber.values.head), NormalMode)(request, messages(application)).toString
      }
    }
