...
import views.html.claim.RefundingCountryView
  ...
  "RefundingCountry Controller" - {
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[RefundingCountryView]
        val formProvider = application.injector.instanceOf[RefundingCountryFormProvider]
    ...
    "must skip RefundingLanguage and set default language when country has single language" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual routes.RefundPeriodController.onPageLoad(models.NormalMode).url
    ...
    "must pre-fill the form when arriving from the RefundingLanguage page and a saved value exists" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.RefundingCountryController.onPageLoad(models.NormalMode).url)
          .withHeaders("Referer" -> routes.RefundingLanguageController.onPageLoad(models.NormalMode).url)
