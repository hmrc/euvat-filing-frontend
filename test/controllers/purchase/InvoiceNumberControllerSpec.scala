  ...
  "InvoiceNumber Controller" - {
    ...
    "must persist and redirect to CYA when in CheckMode and value changed" in {
      ...
      val userAnswers = emptyUserAnswers
        ...
        .success
        .value
        .set(SupplierVatRegistrationNumberPage, "0111")
        .success
        .value
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.InvoiceNumberController.onSubmit(CheckMode).url)
    ...
    "must route to SupplierTaxIdentifierNumber when invoice changed and warning flag set in CheckMode" in {
      val mockSessionRepository = mock[SessionRepository]
      when(mockSessionRepository.set(any())) thenReturn Future.successful(true)
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.InvoiceNumberController.onPageLoad(CheckMode).url)
    ...
    "must route to the next page invoice date" in {
      val mockSessionRepository = mock[SessionRepository]
      when(mockSessionRepository.set(any())) thenReturn Future.successful(true)
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual routes.InvoiceDateController.onPageLoad(NormalMode).url
      }
    }
