...
import models.{CheckMode, FoodAndDrink, Fuel, InvoiceType, Other, PurchaseOrImportType, SupplierTaxNumber, UserAnswers}
import pages.*
    ...
    "currency row in sections" - {
      "must include a currency row when a display name is provided (multi-currency country)" in {
        ...
        running(application) {
          ...
          currencyRow._3.head._1 mustEqual controllers.purchase.routes.RefundingCurrencyController.onPageLoad(CheckMode).url
        }
      }
    }
