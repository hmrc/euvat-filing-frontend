...
class InvoiceNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "InvoiceNumber Controller" - {
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request = FakeRequest(POST, invoiceNumberRoute).withFormUrlEncodedBody(("value", ""))
    ...
    "must redirect to warning when invoice unchanged and warning flag set" in {
      ...
      running(application) {
        val request = FakeRequest(POST, invoiceNumberRoute).withFormUrlEncodedBody(("value", "INV123"))
        val result = route(application, request).value
    ...
    "must route to SupplierTaxIdentifierNumber when invoice changed and warning flag set" in {
      ...
      running(application) {
        val request = FakeRequest(POST, invoiceNumberRoute).withFormUrlEncodedBody(("value", "INV124"))
        val result = route(application, request).value
