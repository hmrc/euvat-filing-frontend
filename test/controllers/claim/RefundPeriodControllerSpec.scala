    ...
    ".onPageLoad" - {
      "must return OK and the correct view for a GET" in {
        ...
        running(application) {
          ...
          val view = application.injector.instanceOf[RefundPeriodView]
    ...
    ".onSubmit" - {
      ...
      "must show both start-and-end-in-same-year and before-earliest when non-exempt VRN and dates span years but are before earliest" in {
        ...
        running(application) {
          ...
          val beforeEarliest = java.time.YearMonth.of(2025, 1).atDay(1).format(java.time.format.DateTimeFormatter.ofPattern("MMMM yyyy"))
          contentAsString(result) must include(beforeEarliest)
      ...
      "must show before-earliest error on start field when non-exempt VRN and start before earliest" in {
        ...
        running(application) {
          ...
          status(result) mustEqual BAD_REQUEST
          val startDate = java.time.YearMonth.of(2021, 1).atDay(1).format(java.time.format.DateTimeFormatter.ofPattern("MMMM yyyy"))
          contentAsString(result) must include(messages(application)("refundPeriod.error.beforeEarliest.start", startDate))
        }
      }
      ...
      "overlap check" - {
        ...
        "must show overlap error when draft exists and submitted status with both matching refund period" in {
          ...
          running(application) {
            ...
            status(result) mustEqual SEE_OTHER
            redirectLocation(result).value mustEqual controllers.warning.routes.PeriodOverlapWarningController.onPageLoad(NormalMode).url
          }
        }
