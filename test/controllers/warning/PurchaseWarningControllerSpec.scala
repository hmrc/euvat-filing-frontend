...
import views.html.warning.PurchaseWarningView
  ...
  "PurchaseWarning Controller" - {
    ...
    "must redirect to InvoiceTypeController on submit in NormalMode" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual controllers.purchase.routes.InvoiceTypeController.onPageLoad(NormalMode).url
      }
    }
