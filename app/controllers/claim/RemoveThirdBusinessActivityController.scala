...
import forms.claim.RemoveBusinessActivityFormProvider
...
import views.html.claim.RemoveBusinessActivityView
...
class RemoveThirdBusinessActivityController @Inject() (
  ...
  private def parseMode(implicit request: play.api.mvc.Request[?]): Mode =
    request
      .getQueryString("mode")
      .orElse(request.session.get(SessionModeKey))
      .flatMap {
      ...
      .fold(
        ...
        },
        {
          case true =>
            ...
            updatedAnswers.flatMap(u =>
              sessionRepository.set(u).map { _ =>
