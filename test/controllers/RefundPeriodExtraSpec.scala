...
class RefundPeriodMatrixSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    ...
    when(mockEuVatRefundsService.retrieveTraderKnownFacts()(any()))
      .thenReturn(Future.successful(TraderKnownFactsResponse(111111111, tradeClass = None)))
  ...
  "RefundPeriod Matrix" - {
    ...
    "ID2: Exempt VRN after latest should show after-latest error" in {
      ...
      val application = appBuilder(userAnswers = Some(userAnswers))
        .configure(
          "settings.refund.can.create.vrns"               -> "222222222",
    ...
    "ID3: Exempt VRN period too short (Oct-Nov 2020) should error" in {
      ...
      val application = appBuilder(userAnswers = Some(userAnswers))
        .configure(
          "settings.refund.can.create.vrns"               -> "333333333",
          "settings.refund.start.date.earliest.permitted" -> "01/20",
          "settings.refund.start.date.latest.permitted"   -> "12/20"
        )
        .build()
